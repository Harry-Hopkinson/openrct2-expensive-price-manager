...
export default class UIWindow {
  private static readonly classification = "expensive-price-manager";
  ...
  public constructor(config: IConfig, priceManager: IPriceManager) {
    ...
    function apply<T extends Widget>(
      name: string,
      action: (widget: T) => void,
    ): void {
      const window = ui.getWindow(UIWindow.classification);
      if (window) action(window.findWidget<T>(name));
    }

    function label(observable: Observable<any>, tabbed = false): LabelDesc {
      return {
        type: "label",
    ...
    function checkbox(
      ...
      observable.observeValue((value) => {
        ...
        apply<CheckboxWidget>(
          observable.name,
          (checkbox) => (checkbox.isChecked = value),
        );
      });
      return desc;
    }
    function dropdown<T extends string>(
      ...
      observable.observeValue((value) => {
        ...
        apply<DropdownWidget>(
          observable.name,
          (dropdown) => (dropdown.selectedIndex = idx),
        );
      });
      return desc;
    }
    ...
    widgets.push(checkbox(config.automaticPriceManagementEnabled));
    advance(space);
    widgets.push(
    ...
    widgets.push(
      ...
      dropdown(config.overchargeUmbrellasPolicy, [
        "always",
        "when it rains",
        "never",
      ]),
    ...
    widgets.push(
      ...
      spinner(
        config.toiletPrice,
        (value) => `${Math.floor(value / 10)}.${value % 10}0`,
        0,
        undefined,
        1,
      ),
    ...
    advance(space);
    widgets.push(checkbox(config.serverWriteAdminOnly));
    advance(space);
    widgets.push(
      button(
        "updatePrices",
    ...
    if (network.mode !== "none") {
      config.serverWriteAdminOnly.observeValue(() =>
        this.updateAuthorisation(),
      );
      context.subscribe("action.execute", (event) => {
        if (event.action === "playersetgroup") this.updateAuthorisation();
      });
      this.updateAuthorisation();
    }
  }
  ...
  private updateAuthorisation(): void {
    const disabled = !Server.isWriteAuthorized(
  ...
  public show(): void {
    ...
    else ui.openWindow(this.windowDesc);
  }
}
