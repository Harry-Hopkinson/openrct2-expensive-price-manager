...
export default class Server {
  ...
  public constructor(config: IConfig, priceManager: IPriceManager) {
    ...
    for (let key in this.config)
      this.config[<keyof IConfig>key].observeValue((value) =>
        this.broadcastValue(<keyof IConfig>key, value),
      );
  ...
  private setValue(args: SetValueActionArgs, player: Player): void {
    if (!Server.isWriteAuthorized(player, this.config)) return;
    if (args.key === "serverWriteAdminOnly" && !Server.isAdmin(player)) return;
...
function find<T>(arr: T[], fun: (val: T) => boolean): T | undefined {
  for (let val of arr) if (fun(val)) return val;
  return undefined;
}
