...
export default class PriceManager implements IPriceManager {
  ...
  public constructor(config: IConfig) {
    ...
    if (config.automaticPriceManagementEnabled.getValue()) this.updatePrices();
  }

  public updatePrices(makeRidesFree: boolean = false): void {
    if (park.getFlag("noMoney")) return;

    // park price management
    this.updateParkPrice();

    // ride and shop price management
    map.rides.forEach((ride: Ride) => this.updatePrice(ride, makeRidesFree));
  }
  ...
  private updateParkPrice(): void {
    if (!this.config.parkPriceManagementEnabled.getValue()) return;

    // Scenario does not allow charging for park entry.
    if (park.getFlag("freeParkEntry")) return;
  ...
  private calculateParkPrice(): number {
    ...
    if (park.getFlag("unlockAllPrices"))
      if (this.config.rct1ChargePolicy.getValue() === "rides") return 0;

    return Math.min(
      park.totalRideValueForMoney,
      this.getMaximumParkEntranceFee(),
    );
  }

  private updatePrice(ride: Ride, makeRidesFree: boolean): void {
    if (ride.price.length === 0) return;
    if (ride.classification === "ride")
      this.updateRidePrice(ride, makeRidesFree);
    else this.updateShopPrice(ride, true);
    if (ride.price.length > 1) this.updateShopPrice(ride, false);
  }

  private updateRidePrice(ride: Ride, makeFree: boolean): void {
    if (!this.config.ridePriceManagementEnabled.getValue()) return;

    // Ignore unrated rides.
    if (ride.excitement < 0) return;
  ...
  private updateShopPrice(ride: Ride, isPrimaryPrice: boolean): void {
    if (!this.config.shopPriceManagementEnabled.getValue()) return;

    // Toilets.
    if (ride.type === 36)
      if (this.config.toiletPriceManagementEnabled.getValue())
        return this.setRidePrice(
          ride,
          this.config.toiletPrice.getValue(),
          isPrimaryPrice,
        );
      else return;

    const price = ShopItem.getItemPrice(
      isPrimaryPrice ? ride.object.shopItem : ride.object.shopItemSecondary,
      this.config,
    );
  ...
  private calculateRidePrice(ride: Ride): number {
    ...
    // RCT1 allows charging for both the park entry and rides.
    if (park.getFlag("unlockAllPrices"))
      if (this.config.rct1ChargePolicy.getValue() === "park entry") return 0;

    // Make transport rides free.
    if (
      this.config.freeTransportRidesEnabled.getValue() &&
      [5, 6, 18, 43, 63].indexOf(ride.type) !== -1
    )
      return 0;
    ...
    const value =
      ride.value >>
      (park.entranceFee > 0 && !park.getFlag("freeParkEntry") ? 2 : 0);

    // Guests will pay at most double the value
    // If it is less than half, they think it is good value
    let price = this.config.goodValueEnabled.getValue()
      ? value >> 1
      : value << 1;

    if (this.config.lazyTaxEnabled.getValue())
      price = Math.floor(
        price * (1 - this.config.lazyTaxFactor.getValue() / 100),
      );
